...
public final class RMSMappingQuality extends InfoFieldAnnotation implements StandardAnnotation, ReducibleAnnotation {
    ...
    public VariantContext finalizeRawMQ(final VariantContext vc) {
        ...
        if (rawMQdata == null) {
            if (!vc.hasAttribute(GATKVCFConstants.MAPPING_QUAL_DEPTH_KEY)) {
                return vc;
            }
            if (vc.hasAttribute(GATKVCFConstants.MAPPING_QUAL_DEPTH_KEY)) {
                final int numOfReads = vc.getAttributeAsInt("MQ_DP", getNumOfReads(vc));  //MQ_DP is an undocumented hack for the Gnarly Pipeline -- improved version uses RAW_MQ_and_DP tuple format (see #4969)
                rawMQdata = vc.getAttributeAsString(getDeprecatedRawKeyName(), null);
                final double squareSum = parseDeprecatedRawDataString(rawMQdata);
    ...
    private static int getNumOfReads(final VariantContext vc) {
        if(vc.hasAttribute(GATKVCFConstants.MAPPING_QUAL_DEPTH_KEY)) {
            int MQDP = vc.getAttributeAsInt(GATKVCFConstants.MAPPING_QUAL_DEPTH_KEY, 0);
            if (MQDP > 0) {
                return MQDP;
            }
        }
